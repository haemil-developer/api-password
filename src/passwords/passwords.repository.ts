...
export class PasswordsRepository {
  ...
  async update(updatePasswordDto: UpdatePasswordDto) {
    ...
    if (!updatePasswordDto.categoryId)
      existingPassword.categoryId = updatePasswordDto.categoryId;
    if (!updatePasswordDto.name) existingPassword.name = updatePasswordDto.name;
    if (!updatePasswordDto.loginId)
      existingPassword.loginId = updatePasswordDto.loginId;
    if (!updatePasswordDto.loginPassword)
      existingPassword.loginPassword = updatePasswordDto.loginPassword;

    await this.save(existingPassword);
  }

  delete(id: number) {
    return this.password.delete(id).catch((e) => {
      throw e;
    });
  }
}
